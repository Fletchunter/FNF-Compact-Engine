...
class Character extends FlxSprite
{
	...
	public function new(x:Float, y:Float, ?character:String = "bf", ?isPlayer:Bool = false)
	{
		...
		switch (curCharacter)
		{
			case 'gf':
				// GIRLFRIEND CODE
				tex = backend.Paths.getSparrowAtlas('characters/GF_assets');
				frames = tex;
			...
			case 'gf-christmas':
				tex = backend.Paths.getSparrowAtlas('characters/gfChristmas');
				frames = tex;
				quickAnimAdd('cheer', 'GF Cheer');
				quickAnimAdd('singLEFT', 'GF left note');
				quickAnimAdd('singRIGHT', 'GF Right Note');
				...
				playAnim('danceRight');
			case 'gf-tankmen':
				frames = backend.Paths.getSparrowAtlas('characters/gfTankmen');
			...
			case 'bf-holding-gf':
				frames = backend.Paths.getSparrowAtlas('characters/bfAndGF');
			...
			case 'gf-car':
				tex = backend.Paths.getSparrowAtlas('characters/gfCar');
				frames = tex;
			...
			case 'gf-pixel':
				tex = backend.Paths.getSparrowAtlas('characters/gfPixel');
				frames = tex;
			...
			case 'dad':
				...
				tex = backend.Paths.getSparrowAtlas('characters/DADDY_DEAREST');
				frames = tex;
				...
				playAnim('idle');
			case 'spooky':
				tex = backend.Paths.getSparrowAtlas('characters/spooky_kids_assets');
				frames = tex;
				...
				playAnim('danceRight');
			case 'mom':
				tex = backend.Paths.getSparrowAtlas('characters/Mom_Assets');
				frames = tex;
			...
			case 'mom-car':
				tex = backend.Paths.getSparrowAtlas('characters/momCar');
				frames = tex;
				...
				playAnim('idle');
			case 'monster':
				tex = backend.Paths.getSparrowAtlas('characters/Monster_Assets');
				frames = tex;
			...
			case 'monster-christmas':
				tex = backend.Paths.getSparrowAtlas('characters/monsterChristmas');
				frames = tex;
				quickAnimAdd('idle', 'monster idle');
				quickAnimAdd('singUP', 'monster up note');
				quickAnimAdd('singDOWN', 'monster down');
				...
				playAnim('idle');
			case 'pico':
				tex = backend.Paths.getSparrowAtlas('characters/Pico_FNF_assetss');
				frames = tex;
			...
			case 'pico-speaker':
				frames = backend.Paths.getSparrowAtlas('characters/picoSpeaker');
			...
			case 'bf':
				var tex = backend.Paths.getSparrowAtlas('characters/BOYFRIEND');
				frames = tex;
				quickAnimAdd('idle', 'BF idle dance');
				quickAnimAdd('singUP', 'BF NOTE UP0');
				quickAnimAdd('singLEFT', 'BF NOTE LEFT0');
			...
			case 'bf-christmas':
				var tex = backend.Paths.getSparrowAtlas('characters/bfChristmas');
				frames = tex;
				quickAnimAdd('idle', 'BF idle dance');
				quickAnimAdd('singUP', 'BF NOTE UP0');
				quickAnimAdd('singLEFT', 'BF NOTE LEFT0');
				...
				flipX = true;
			case 'bf-car':
				var tex = backend.Paths.getSparrowAtlas('characters/bfCar');
				frames = tex;
				quickAnimAdd('idle', 'BF idle dance');
				quickAnimAdd('singUP', 'BF NOTE UP0');
				quickAnimAdd('singLEFT', 'BF NOTE LEFT0');
				...
				flipX = true;
			case 'bf-pixel':
				frames = backend.Paths.getSparrowAtlas('characters/bfPixel');
				...
				flipX = true;
			case 'bf-pixel-dead':
				frames = backend.Paths.getSparrowAtlas('characters/bfPixelsDEAD');
			...
			case 'bf-holding-gf-dead':
				frames = backend.Paths.getSparrowAtlas('characters/bfHoldingGF-DEAD');
			...
			case 'senpai':
				frames = backend.Paths.getSparrowAtlas('characters/senpai');
			...
			case 'spirit':
				frames = backend.Paths.getPackerAtlas('characters/spirit');
			...
			case 'parents-christmas':
				frames = backend.Paths.getSparrowAtlas('characters/mom_dad_christmas_assets');
				...
				playAnim('idle');
			case 'tankman':
				frames = backend.Paths.getSparrowAtlas('characters/tankmanCaptain');
	...
	{
		var swagshit = backend.Song.loadFromJson('picospeaker', 'stress');
	...
	{
		var daFile:Array<String> = backend.CoolUtil.coolTextFile(backend.Paths.file("images/characters/" + offsetCharacter + "Offsets.txt"));
	...
	override function update(elapsed:Float)
	{
		if (!curCharacter.startsWith('bf'))
		{
			...
			if (curCharacter == 'dad')
				dadVar = 6.1;
			if (holdTimer >= backend.Conductor.stepCrochet * dadVar * 0.0011)
			{
				dance();
				holdTimer = 0;
			}
		...
		switch (curCharacter)
		{
			...
			case "pico-speaker":
				...
				{
					if (backend.Conductor.songPosition > animationNotes[0][0])
					{
