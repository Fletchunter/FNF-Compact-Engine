...
class TankmenBG extends FlxSprite
{
	...
	public function new(x:Float, y:Float, isGoingRight:Bool)
	{
		...
		frames = backend.Paths.getSparrowAtlas('tankmanKilled1');
		antialiasing = true;
	...
	override function update(elapsed:Float)
	{
		...
		if (animation.curAnim.name == 'run')
		{
			...
			if (goingRight)
			{
				...
				x = (endDirection + (backend.Conductor.songPosition - strumTime) * tankSpeed);
			}
			else
			{
				x = (endDirection - (backend.Conductor.songPosition - strumTime) * tankSpeed);
		...
		if (backend.Conductor.songPosition > strumTime)
		{
			// kill();
			animation.play('shot');
