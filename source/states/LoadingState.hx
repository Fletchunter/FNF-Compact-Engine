...
class LoadingState extends backend.MusicBeatState
{
	...
	override function create()
	{
		...
		funkay.loadGraphic(backend.Paths.image('funkay'));
		...
		initSongsManifest().onComplete(function(lib)
		{
			...
			checkLibrary("shared");
			if (states.PlayState.storyWeek > 0)
				checkLibrary("week" + states.PlayState.storyWeek);
			else
	...
	{
		return backend.Paths.inst(states.PlayState.SONG.song);
	}

	static function getVocalPath()
	{
		return backend.Paths.voices(states.PlayState.SONG.song);
	}
	...
	{
		backend.Paths.setCurrentLevel("week" + states.PlayState.storyWeek);
		#if NO_PRELOAD_ALL
		var loaded = isSoundLoaded(getSongPath())
			&& (!states.PlayState.SONG.needsVoices || isSoundLoaded(getVocalPath()))
