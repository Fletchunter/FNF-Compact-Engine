...
class MainMenuState extends backend.MusicBeatState
{
	...
	override function create()
	{
		...
		var bg:FlxSprite = new FlxSprite(backend.Paths.image('menuBG'));
		bg.scrollFactor.x = 0;
		...
		magenta = new FlxSprite(backend.Paths.image('menuDesat'));
		...
		FlxG.cameras.reset(new backend.SwagCamera());
	...
	function onMenuItemChange(selected:states.MainMenuState.MainMenuItem)
	{
	...
	{
		// FlxG.camera.followLerp = backend.CoolUtil.camLerpShit(0.06);
...
private class MainMenuList extends MenuTypedList<MainMenuItem>
{
	...
	public function new()
	{
		atlas = backend.Paths.getSparrowAtlas('main_menu');
		super(Vertical);
	}
