...
class CutsceneCharacter extends FlxTypedGroup<FlxSprite>
{
	...
	{
		var splitShit:Array<String> = backend.CoolUtil.coolTextFile(backend.Paths.file('images/cutsceneStuff/' + imageShit + "CutsceneOffsets.txt"));
	...
	public function createCutscene(daNum:Int = 0)
	{
		...
		cutScene.frames = backend.Paths.getSparrowAtlas('cutsceneStuff/' + imageShit + "-" + daNum);
